...
impl KalshiMarketReader {
    pub fn new(
        ...
        tokio::task::spawn(async move {
            ...
            const TICKS_BETWEEN_RESOLVE_POLLS: u64 = TIME_BETWEEN_RESOLVE_POLLS_MS / TIME_BETWEEN_TICKS_MS;
            ...
            fn handle_incoming_websocket_message(
                ...
                if let Ok(ws_msg) = serde_json::from_str::<KalshiWsMessage>(&message) {
                    match ws_msg {
                        KalshiWsMessage::OrderbookSnapshot { msg, .. } => {
                        ...
                        KalshiWsMessage::OrderbookDelta { msg: DeltaMsg { price_dollars, delta_fp, side }, .. } => {
                ...
                tokio::select! {
                    ...
                    _ = interval.tick() => {
                        let mut connect_to_websocket = async || {
                            if web_socket.is_none() {
                                web_socket = Some(
                                    connect_ws(&[ticker.0.clone()], &api_key_id, &priv_key_path).await.unwrap()
                                );
                            }
                        };
                        ...
                        let mut poll_if_not_recent = async || {
                            ...
                            if ticks_since_last_resolve_poll > TICKS_BETWEEN_RESOLVE_POLLS {
                                ...
                                let now = Utc::now();
                                let nearby_markets = super::poll_nearby_markets(&rest_client, now).await;
                    ...
                    web_socket_message = async {
                        ...
                        match web_socket_message {
                            Some(Ok(message)) => {
                                handle_incoming_websocket_message(message.to_string(), &mut output_tx, &on_update);
                            }
        ...
        KalshiMarketReader { input_tx, output_rx, state_cache: initial_state }
    }
...
enum KalshiWsMessage {
    #[serde(rename = "orderbook_snapshot")] OrderbookSnapshot { msg: SnapshotMsg },
    #[serde(rename = "orderbook_delta")] OrderbookDelta { msg: DeltaMsg },
    #[serde(other)] Unknown
}

#[derive(Debug, Deserialize)]
struct SnapshotMsg {
    #[serde(default)] pub yes_dollars_fp: Vec<[String; 2]>,
    #[serde(default)] pub no_dollars_fp:  Vec<[String; 2]>
}

#[derive(Debug, Deserialize, PartialEq, Eq, PartialOrd, Ord)]
enum KalshiMarketSide { #[serde(rename = "yes")] Yes, #[serde(rename = "no")] No }

#[derive(Debug, Deserialize)]
struct DeltaMsg {
    #[serde(default, deserialize_with = "super::deserialize_stringified_float")] pub price_dollars: f64,
    #[serde(default, deserialize_with = "super::deserialize_stringified_float")] pub delta_fp:      f64,
    pub side: KalshiMarketSide
}

#[derive(Clone, PartialEq, Eq, PartialOrd, Ord)]
pub enum MarketPollState {
    FarBeforeActive, RightBeforeActive, ActiveLookingForStrike, ActiveKnownStrike, ActivelyTryingToResolve, Resolved
}

pub enum MarketStreamEvent {
    OrderbookSnapshot(Orderbook),
    OrderbookDelta { price_dollars: f64, size_delta: i32 },
    NewDescriptors(Vec<KalshiMarketDescriptor>)
}

pub async fn connect_ws(tickers: &[String], api_key_id: &str, priv_key_path: &str) -> Result<WebSocketStream<MaybeTlsStream<TcpStream>>> {
    ...
    h.insert("KALSHI-ACCESS-SIGNATURE", HeaderValue::from_str(&BASE64.encode(signature.to_vec()))?);
    ...
    ws.send(Message::Text(
        json!({ "id": 1, "cmd": "subscribe", "params": { "channels": ["orderbook_delta"], "market_tickers": tickers } }).to_string().into()
    )).await?;
    Ok(ws)
}